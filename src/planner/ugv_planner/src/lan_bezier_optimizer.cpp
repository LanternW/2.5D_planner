...
int lanBezierOptimizer::bezierCurveGeneration( 
...
#define ENFORCE_ACC 0 
    ...
    vector<double>   time = time_allocation;

    int segment_num  = corridors.size();
    ...
    for(int i = 0 ; i < segment_num * s1d1CtrlP_num ; i++)
    {
        con_eq_bd.push_back(0.0);
    }
...
//  ### setting bounds on optimized variables
    ...
    //   Start position 
    {
        ...
        for(int i = 0 ; i < segment_num; i++)
        {
            for(int j = 0 ; j < s1d1CtrlP_num; j++)
            {
                ...
                jcolA[nn_idx] = i * s1CtrlP_num + 2 * s1d1CtrlP_num + j;
                row_idx ++;
                nn_idx  ++;
            }
        }
    ...
    for(int k = 0; k < segment_num; k++) 
    {   
        int p_k = corridors[k].getConstraintNum(); // hyperplane num of this polyhedra
        MatrixXd A = corridors[k].getA();
        ...
        {   
            ...
            {   
                ...
                clow[m_idx]  = 0;
                cupp[m_idx]  = - A.row(j)(2); // plane equation: ax + by + cz + K < 0 ==> ax + by + cz < -K
    ...
    {   
        ...
        {  
            for(int j = 0; j < p_k; j++)
            {   
                dC[nn_idx]   = A.row(j)(0) * val0;
                dC[nn_idx+1] = A.row(j)(1) * val0;
                dC[nn_idx+2] = 0 * val0;
...
//  ######################################################
    ...
    const int nnzQ = 3 * segment_num * (traj_order + 1) * (traj_order + 2) / 2  + segment_num * (traj_order + 1);
    //                                                                         // security soft constraints
    int    irowQ[nnzQ]; 
    int    jcolQ[nnzQ];
    double    dQ[nnzQ];
    ...
    // Soft security constraints
    // cost : distance to SMC boundary

    /*
    double weight_security = 2.0;
    for(int s = 0; s < segment_num; s++)
    {
        Eigen::MatrixXd A = corridors[s].getA();
        int n = corridors[s].getConstraintNum();
        double penalty_num_x = 0.0;
        double penalty_num_y = 0.0;
        double penalty_num_z = 0.0;
        ...
        for(int j = 0 ; j < s1d1CtrlP_num; j++)
        {
            c[s * s1CtrlP_num + 0 * s1d1CtrlP_num + j] = weight_security * penalty_num_x;
            c[s * s1CtrlP_num + 1 * s1d1CtrlP_num + j] = weight_security * penalty_num_y;
            c[s * s1CtrlP_num + 2 * s1d1CtrlP_num + j] = weight_security * penalty_num_z;
        }   

    }
    */
   //this doesn't work , any point in convex get same value. This soft constraint should added in Quadratic term
    ...
    {   
        ...
        {
            A = corridors[k].getA();
            n = corridors[k].getConstraintNum();
            penalty_num_x = 0.0;
            penalty_num_y = 0.0;
            for(int i = 0 ; i < n ; i++)
            {
                penalty_num_x   -= security_weight * pow(A(i,0),2);
                penalty_num_y   -= security_weight * pow(A(i,1),2);
                penalty_num_xy  -= security_weight * 2*A(i,0)*A(i,1);
            }
            ...
            for(int p = 0; p < 3; p ++ )
            {
                for( int i = 0; i < s1d1CtrlP_num; i ++ )
                {
                    for( int j = 0; j < s1d1CtrlP_num; j ++ )
                    {
                        if( i >= j )
                        {
                            ...
                            if(min_order_l == min_order_u)
                                dQ[idx] = MQM_u(i, j) / (double)pow(scale_k, 2 * min_order_u - 3) * pow(1, 2 * min_order_u - 1);
                            else
                                dQ[idx] = MQM_l(i, j) / (double)pow(scale_k, 2 * min_order_l - 3) * pow(1, 2 * min_order_l - 1)
                                        + MQM_u(i, j) / (double)pow(scale_k, 2 * min_order_u - 3) * pow(1, 2 * min_order_u - 1);
                            
                            if(i == j)
                            {
                                //x-axis
                                if(p == 0)
                                    dQ[idx] += penalty_num_x;

                                //y-axis
                                else if (p == 1)
                                    dQ[idx] += penalty_num_y;
                            }
                            idx ++ ;
                        }
                    }
                    ...
                    {
                        irowQ[idx] = sub_shift + p * s1d1CtrlP_num + i;   
                        jcolQ[idx] = sub_shift + p * s1d1CtrlP_num + i + s1d1CtrlP_num; 
                        dQ[idx]    = penalty_num_xy;
                        idx ++;
                    }    
    ...
    if( ierr == 0 ) 
    {
        ...
        {   
            ...
            double scale = time[i];
            if(min_order_l == min_order_u)
                Q_o = MQM_u / (double)pow(scale, 2 * min_order_u - 3) * pow(1, 2 * min_order_u - 1);
            else
                Q_o = MQM_l / (double)pow(scale, 2 * min_order_l - 3) * pow(1, 2 * min_order_l - 1)
                    + MQM_u / (double)pow(scale, 2 * min_order_u - 3) * pow(1, 2 * min_order_u - 1);
